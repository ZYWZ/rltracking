...
class StableTransformerXL(torch.nn.Module):
    ...
    def init_memory(self, input, device=torch.device("cpu")):
        return [
            # torch.empty(0, dtype=torch.float).to(device)
            torch.zeros(input.shape[0], input.shape[1], input.shape[2], dtype=torch.float).to(device)
    ...
    def forward(self, inputs, memory=None):
        ...
        """
        if memory is None:
            memory = self.init_memory(inputs, inputs.device)
